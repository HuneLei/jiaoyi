<template>
  <div>
    ...
    <div v-loading="loading">
      <el-form :rules="editMessage.rules" label-width="100px" id="from-item">
        <TradeEditsFormI :authbtn="authbtn" :from-edit="totalForm" ref="changeBtnStatus" :isCreatShow="middleBtnShow.isCreatShow" :isDeleteItem="middleBtnShow.isDeleteItem" :tradeEditBtn="tradeEditBtn" :message="editMessage" :isGoBackShow="false" :isExportShow="middleBtnShow.isExportShow" :isImportShow="middleBtnShow.isImportShow" :arrow-num=2 :arrow-down="true" @data-creat="dataCreat" @data-export="dataExport" @data-import="dataImport" @delete-item="deleteItem" @show-more="tableHeightRun">
          <el-form-item label="联系人" prop="gpoCodeContacter" slot='gpoCodeContacter'>
            <el-input v-model="totalForm.gpoCodeContacter" readonly disabled size="small" style="width: 100%; min-width: 120px;"></el-input>
          </el-form-item>
          ...
          <el-form-item label="选择卖方会员" prop="salerMemberId" slot='salerMemberId'>
            <el-select v-model="totalForm.salerMemberId" filterable placeholder="请选择" size="small" style="width: 100%; min-width: 120px;">
          ...
          <!--<el-button size="mini" type="primary" style="line-height: 20px;" @click="addAdress" v-if="View_UserInfo.memberType !== 3 && View_UserInfo.memberType !== 5" v-show="authbtn1.create">-->
    ...
    <DialogForm :dialog-form-visible="dialogFormShow" @close-isSubmitShowog="CloseDialog" @dialog-cancel="dialogCancel" :message="AddressMessage" :from-dialog="form" :isReadonly="false" :isSaveShow="true" :saveLoading="false" :isFormConfirm="isFormConfirm" @data-save="dataSave">
    ...
    return {
      ...
      editMessage: {
        ...
        selects: [
          ...
          { value: 'gpoCodeContacter', type: 'custom', span: 5 },
  ...
  methods: {
      ...
      GpoPurchaseList.listItem(this.topbarForm).then((res) => {
        if (res.data.code === 0) {
          ...
          if (data[0]) {
            ...
            this.totalForm.gpoCodeContacter = `${createBy} ${createTime}`; // GPO采购单编号
            this.totalForm.gpoContacter = `${gpoPerson} ${gpoPhone}`;      //  GPO
            ...
            this.totalForm.id = data[0].id;
            this.totalForm.gpoMemberId = this.getGpoId;
            // this.totalForm.projectId = Number(data.content[0].projectId);
            ...
            this.totalForm.receiveAddressId = data[0].receiveAddressId;
            this.totalForm.remarks = data[0].remarks;
            this.totalForm.salerMemberCode = data[0].code;   // 卖方会员编码
            this.totalForm.salerMemberId = Number(data[0].salerMemberId);       // 卖方会员ID
            this.totalForm.salerMemberName = data[0].salerMemberName;   // 卖方会员名称
      ...
      GpoPurchaseList.drugsList(obj).then((res) => {
        if (res.data.code === 0) {
          ...
          for (let i = 0; i < data.length; i += 1) {
            ...
            data[i].gpoPrice = data[i].gpoPrice;
    ...
    changeSalerMemberId(val, oldVal) {
      ...
      if (val) {
        ...
        if (oldVal && this.totalForm.gpoPurchaseDrugsItemFormList.length > 0) {
      ...
      //      // 父组件触发子组件的改变按钮状态的事件
      //      if (this.totalForm.salerMemberId) {
      //        this.tradeEditBtn = {
      //          createBtn: false,        // 新增
      //          deleteBtn: true,         // 删除
      //          exportBtn: false,        // 导出
    ...
    priceChange(data) {  // 数量发生变化的时候触发
      this.editStatus = 1;
      this.totalForm.gpoPurchaseDrugsItemFormList[data.$index].purchaseAmount =
        (Number(data.row.purchaseNum) * parseFloat(data.row.gpoPrice)).toFixed(2);
      let amount = 0;
      let totalNum = 0;
      for (let i = 0; i < this.totalForm.gpoPurchaseDrugsItemFormList.length; i += 1) {
        amount += parseFloat(this.totalForm.gpoPurchaseDrugsItemFormList[i].purchaseAmount);
        totalNum += Number(this.totalForm.gpoPurchaseDrugsItemFormList[i].purchaseNum);
      }
      this.totalForm.amount = amount.toFixed(2);
      this.totalForm.totalNum = totalNum;
    ...
    deleteItem() {
      ...
      for (let i = 0; i < this.totalForm.gpoPurchaseDrugsItemFormList.length; i += 1) {
        for (let j = 0; j < this.selectedDrugs.length; j += 1) {
          if (this.totalForm.gpoPurchaseDrugsItemFormList[i].drugsId === this.selectedDrugs[j]) {
    ...
    toAjax() {
      ...
      if (goCreate) {
        this.loading = true;
        GpoPurchaseList.update(this.totalForm).then((res) => {
          if (res.data.code === 0) {
            this.$message({
              type: 'success',
              message: '操作成功',
    ...
    orderDelete() { // 删除按钮触发事件
      ...
      GpoPurchaseList.deleteP(arrId).then((res) => {
        if (res.data.code === 0) {
          const data = res.data.result;
          this.$message({
            type: 'success',
