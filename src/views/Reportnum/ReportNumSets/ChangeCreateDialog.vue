  ...
  <div>
    <DialogForm :dialog-form-loading="DialogFormLoading" :dialog-form-visible="dialogFormShow" @close-dialog="CloseDialog" @dialog-confirm="dialogConfirm" @dialog-cancel="dialogCancel" :message="editMessage" :from-dialog="form" :isReadonly="false" :isFormConfirm="true" :formConfirmLoading="formConfirmLoading">
      <el-form-item label="集采项目" prop="projectId" slot='projectId' label-width="115px">
        <el-select v-model="projectId" placeholder="请选择集采项目" filterable @change="getSelectValue">
      ...
      <el-form-item label="集采子项目" prop="projectSubId" slot='projectSubId' label-width="115px">
        <el-select v-model="form.projectSubId" multiple filterable placeholder="请选择集采子项目">
...
export default {
  activated() {
    this.$bus.$on('change/create/dialog', this.dataCreate);
  },
  deactivated() {
    this.$bus.$off('change/create/dialog', this.dataCreate);
  },
  components: {
    DialogForm,
  },
  watch: {
    ...
    projectSubId() {
      this.form.projectSubId = this.projectSubId;
    },
  },
  computed: {
    ...mapGetters([
  ...
  methods: {
    ...
    dialogConfirm(formName) {
      ...
      ReportNumSets.create(this.getGpoId, this.form.projectId, this.form.projectSubId,
        this.form.timeName, this.form.reportnumBtime,
