<template>
  <div>
    <DialogForm :dialog-form-loading="DialogFormLoading" :dialog-form-visible="dialogFormShow" @close-dialog="CloseDialog" @dialog-confirm="dialogConfirm" @dialog-cancel="dialogCancel" :message="editMessage" :from-dialog="form" :isFormConfirm="true" :formConfirmLoading="formConfirmLoading">
      <el-form-item label="发票" prop="scanFile" slot='scanFile' label-width="115px">
...
<script>
  ...
  export default {
    activated() {
      this.$bus.$on('invoiceEdit/dialog', this.dataUpdate);
    },
    deactivated() {
      this.$bus.$off('invoiceEdit/dialog', this.dataUpdate);
    },
    components: {
      DialogForm,
      YUploadEdit,
    ...
    data() {
      return {
        ...
        editMessage: {
          ...
          rules: {
            scanFile: [
              { required: true, message: '请上传发票', validator: window.validator.enclosure, trigger: 'change' },
            ],
          },
          styleObject: {},
        },
    ...
    methods: {
      ...
      dataUpdate(row) {
        ...
        InvoiceList.scanningItem(row.id).then((res) => {
          if (res.data.code === 0) {
      ...
      dialogConfirm(formName) {
        InvoiceList.scanningUpdata(this.form.id, this.form.scanFile).then((res) => {
          if (res.data.code === 0) {
