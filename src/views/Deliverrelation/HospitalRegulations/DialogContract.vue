...
export default {
  activated() {
    this.$bus.$on('data/choose/DialogContract', this.dataChoose);
  },
  deactivated() {
    this.$bus.$off('data/choose/DialogContract', this.dataChoose);
  },
  created() {
    this.tableHeightRun();
    window.onresize = () => {
  ...
  methods: {
    ...
    dataDialogList(msg, form, page, pageSizes) {
      ...
      HospitalRegulations.contractList(cpage, cpageSizes, forms)
        .then((res) => {
          if (res.data.code === 0) {
