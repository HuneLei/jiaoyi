    ...
    <el-dialog :visible.sync="dialogVisible" size="small">
      <el-form ref="form" label-width="140px" class="edit-form">
        ...
        <el-table v-loading="storeLoading" border fit :data="storeData" stripe height="150">
          ...
          <el-table-column min-width="120" label="证明文件" sortable class-name="table_right">
            <template scope="scope">
...
export default {
  activated() {
    this.$bus.$on('data/area/create1', this.dataCreate);
  },
  deactivated() {
    this.$bus.$off('data/area/create1', this.dataCreate);
  },
  data() {
    return {
      elTabs: 0,
