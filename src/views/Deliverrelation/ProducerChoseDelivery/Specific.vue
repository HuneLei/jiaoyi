...
export default {
  activated() {
    this.$bus.$on('data/getId', this.getId);    // 监听获取已选择配送商的列表结果
    // this.dataList();
  },
  deactivated() {
    this.$bus.$off('data/getId', this.getId);
  },
  computed: mapGetters([
    'getGpoId',
    'getProjectId',
  ...
  methods: {
    getId(form) {
      ...
      if (form.nameType === 1) {
        this.title = '已建立配送协议的配送企业-医院机构 -- 已选';
      } else if (form.nameType === 0) {
        this.title = '已建立配送协议的配送企业-医院机构 -- 未选';
      } else {
        this.title = '已建立配送协议的配送企业-医院机构 -- 全部';
      }
