      ...
      <el-table :data="tableData" @selection-change="dataSelect" stripe border fit :height="View_TableHeight" @sort-change="sortChange">
        ...
        <el-table-column prop="deliveryMemberName" min-width="120" label="配送企业" show-overflow-tooltip>
          <template scope="scope">
            <el-button type="text" @click="choose(scope.row)" v-if="$route.query.salerStatic === 0">选择 </el-button>
...
import Enterprise from './Enterprise';                    //  选择配送企业
...
export default {
  ...
  watch: {
    ...
    getGpoId() {
      // 当切换gpo时，重定向到列表页
      this.$router.push({ path: '/Deliverrelation/ProducerContinueChoseDelivery' });
    },
    projectId(newinfo, oldinfo) {
      ...
      if (this.$route.path === '/Deliverrelation/ProducerContinueChoseDelivery/Edit') {
    ...
    return {
      ...
      editMessage: {
        selects: [
          ...
          { label: '卖方会员', key: 'salerMemberName', type: 'el-input', placeholder: '', isReadonly: true, span: 5 },
          ...
          { label: '集采项目名称', key: 'projectId', type: 'el-select', option: 'projectOption', span: 5, isReadonly: true },
          ...
          { label: '审核人', key: 'auditBy', type: 'el-input', placeholder: '', isReadonly: true, span: 5 },
          ...
          { label: '备注', key: 'remarks', span: 15, type: 'el-input', placeholder: '请输入备注', fromType: 'textarea', rows: 1, isReadonly: true },
        ],
        tabTitle: '',
        styleObject: {
          style1: {
  ...
  methods: {
    ...
    deliveryAgree(select) {
      ...
      ProducerContinueChoseDelivery.audit(form).then((res) => {
        if (res.data.code === 0) {
          this.$message({
            type: 'success',
            message: '操作成功',
    ...
    dataInit() {
      ...
      if (this.$route.query.id) {
        ProducerContinueChoseDelivery.header(this.$route.query.id)
          .then((res) => {
            if (res.data.code === 0) {
              ...
              this.projectIdsss = data.projectId;
              this.salerMemberIdsss = data.salerMemberId;
              // 附件信息
              ...
              if (this.View_UserInfo.memberType !== 3 && data.salerStatic === 0) {
    ...
    dataList() {
      ...
      if (this.$route.query.id) {
        ProducerContinueChoseDelivery.item(this.$route.query.id)
          .then((res) => {
            if (res.data.code === 0) {
    ...
    alreadyChoosed(row) {
      ...
      if (this.$route.query.salerStatic === 0) {
        ...
        this.dialogOptionsTwo.projectId = this.projectIdsss;
      }
      this.dialogOptionsTwo.drugsId = row.drugsId;
      this.dialogOptionsTwo.delivearrid = row.delivearrid;
      this.dialogOptionsTwo.itemId = row.itemId;
      this.dialogOptionsTwo.salerMemberId = this.salerMemberIdsss;
    ...
    orderDelete() {
      if (this.$route.query.id) {
        ProducerContinueChoseDelivery.destoryTrade(this.$route.query.id).then((res) => {
          // console.log();
          if (res.data.code === 0) {
            this.$message({
              type: 'success',
