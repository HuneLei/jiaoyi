<template>
  <div v-loading="loading">
    <el-table :data="tableData" v-loading="loading" :height="tableHeight" stripe border fit @sort-change="sortChange">
      ...
      <el-table-column align="left" prop="salerMemberName" label="卖方会员" sortable="custom" min-width="180" show-overflow-tooltip>
      </el-table-column>
      ...
      <el-table-column align="left" prop="createBy" label="创建人" sortable="custom" width="120" show-overflow-tooltip>
      </el-table-column>
      ...
      <el-table-column align="left" prop="auditBy" label="审核人" sortable="custom" width="100" show-overflow-tooltip>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    dataList(page, pageSizes) {
      ...
      ProducerContinueChoseDelivery.list(cpage, cpageSizes, form).then((res) => {
        if (res.data.code === 0) {
    ...
    // 详情
    dataUpdate(row) {
      this.$router.push({ path: '/Deliverrelation/ProducerContinueChoseDelivery/Edit', query: { id: row.ids, salerStatic: row.salerStatic } });
    },
    // 页码切换
    pageChange(page) {
      this.dataList(page);
