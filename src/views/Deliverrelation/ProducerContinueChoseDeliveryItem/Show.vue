<template>
  <div>
    <el-table :data="tableData" v-loading="loading" :height="tableHeight" @selection-change="dataSelect" stripe border fit @sort-change="sortChange">
      ...
      <el-table-column prop="idDeliveryAffirm" label="确认状态" align="left" width="80" sortable="custom" show-overflow-tooltip>
        <template scope="scope">
          ...
          <span v-if="scope.row.idDeliveryAffirm === 2" style="color: red">已拒绝</span>
        </template>
      </el-table-column>
      <el-table-column prop="createTime" label="创建时间" align="left" min-width="140" sortable="custom" show-overflow-tooltip>
      </el-table-column>
      ...
      <el-table-column label="操作" align="center" min-width="150" max-width="200" fixed="right" v-if="authbtn.confirm || authbtn.cancel ||authbtn.refuse">
        <template scope="scope">
          <el-button @click="affirm(scope.row, scope.$index)" type="text" size="small" :disabled="scope.row.idDeliveryAffirm === 2 || scope.row.deliveryBtimeType === 0">{{scope.row.idDeliveryAffirm === 1 ? '取消确认': '确认'}}</el-button>
          <el-button @click="refuse(scope.row, scope.$index)" type="text" size="small" :disabled="scope.row.idDeliveryAffirm === 1 || scope.row.deliveryBtimeType === 0">{{scope.row.idDeliveryAffirm === 2 ? '取消拒绝': '拒绝'}}</el-button>
        </template>
      </el-table-column>
    </el-table>
  </div>
...
import ProducerContinueChoseDeliveryItem from '../../../api/Deliverrelation/ProducerContinueChoseDeliveryItem';  // 药品确认api
...
export default {
  ...
  watch: {
    getGpoId() {
      if (this.$route.path === '/Deliverrelation/ProducerContinueChoseDeliveryItem') {
        this.dataList();
      }
    },
  },
  ...
  methods: {
    ...
    affirmChoose() {
      ...
      ProducerContinueChoseDeliveryItem.confirm(this.form).then((res) => {
        // console.log();
        if (res.data.code === 0) {
          this.$message({
            type: 'success',
    ...
    affirm(row, num) {
      ...
      if (this.tableData[num].idDeliveryAffirm) {
        ProducerContinueChoseDeliveryItem.cancel(this.form).then((res) => {
          // console.log();
          if (res.data.code === 0) {
            this.$message({
              type: 'success',
    ...
    refuse(row, num) {
      ...
      if (this.tableData[num].idDeliveryAffirm) {
        ...
        ProducerContinueChoseDeliveryItem.refuse(this.form).then((res) => {
          // console.log();
          if (res.data.code === 0) {
            this.$message({
              type: 'success',
    ...
    dataSearch(data) {
      ...
      this.filterForm = {
        ...
        packSpecName: data.packSpecName.value,
    ...
    dataList(page, pageSizes) {
      ...
      let createTime = '';
      if (this.filterForm.createTime && typeof this.filterForm.createTime === 'object') {
        createTime = `${this.filterForm.createTime[0].replace(/\//g, '-')} - ${this.filterForm.createTime[1].replace(/\//g, '-')}`;
      } else {
        createTime = this.filterForm.createTime;
      }
      if (createTime === '1970-01-01 - 1970-01-01') {
        this.filterForm.createTime = null;
      } else {
        this.filterForm.createTime = createTime;
      }
      // console.log();
      ProducerContinueChoseDeliveryItem.list(cpage, cpageSizes, this.filterForm)
        .then((res) => {
          // console.log();
          if (res.data.code === 0) {
    ...
    dataSelect(row) {
      ...
      for (let i = 0; i < row.length; i += 1) {
        if (Number(row[i].idDeliveryAffirm) === 0 && Number(row[i].deliveryBtimeType) === 1) {
    ...
    dataExport() {
      ...
      const url = ProducerContinueChoseDeliveryItem.output(form);
      window.open(url);
    },
    // 关闭弹出框
    closeDialog() {
