...
export default {
  activated() {
    this.$bus.$on('reufseContract/dialog', this.dataCreate);
  },
  deactivated() {
    this.$bus.$off('reufseContract/dialog', this.dataCreate);
  },
  components: {
    DialogForm,
  },
  ...
  data() {
    return {
      ...
      editMessage: {
        dialogTitle: '拒绝合同',
        selects: [
          { label: '拒绝原因', key: 'reason', type: 'el-select', placeholder: '请选择拒绝合同原因', option: 'reasonOption', noClearable: true },
          { label: '', key: 'remark', type: 'el-input', placeholder: '请输入拒绝合同原因', fromType: 'textarea', rows: 4 },
        ],
        options: {
          reasonOption: [],
        },
        ...
        rules: {
          reason: [
            { required: true, message: '请填写拒绝原因', trigger: 'change', validator: window.validator.select },
          ],
        },
        styleObject: {},
      },
    ...
    dialogConfirm() {
      this.$confirm('确定要拒绝此合同?', '提示', {
        ...
        this.closeObj = Object.assign({}, this.closeObj, {
          ...
          status: this.$route.query.contractStatus,
        });
        TriContractList.refuse(this.closeObj).then((res) => {
          if (res.data.code === 0) {
            this.$message({
              type: 'success',
              message: '拒绝成功',
