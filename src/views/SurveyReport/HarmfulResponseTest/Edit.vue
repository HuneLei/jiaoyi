<template>
  <div v-loading="loading" class="harmfulResponseTest">
    ...
    <TradeFooter class="trade" :authbtn="authbtn" :isSaveShow="isSaveShow" :isBackShow="true" :isDeleteShow="isDeleteShow" @order-save="orderSave('form')" @order-delete="orderDelete" @order-back="orderBack">
    </TradeFooter>
  </div>
</template>
  ...
  methods: {
    dataList() {
      if (this.$route.query.id) {
        HarmfulResponseTest.item(this.$route.query.id).then((res) => {
          if (res.data.code === 0) {
            const data1 = res.data.result;
            if (data1.validTime) {
              const validTime1 = data1.validTime.slice(0, 10);
              const validTime2 = data1.validTime.slice(12, 23);
              this.form.validTime = [`${validTime1}`, `${validTime2}`];
            }
            this.form = {
              batchNo: data1.batchNo,
              contacts: data1.contacts,
              createBy: data1.createBy,
              createTime: data1.createTime,
              disposeInfo: data1.disposeInfo,
              drugsName: data1.drugsName,
              fillTime: data1.fillTime,
              formName: data1.formName,
              id: data1.id,
              notUntowardEffect: data1.notUntowardEffect,
              phone: data1.phone,
              producerName: data1.producerName,
              readyUntowardEffect: data1.readyUntowardEffect,
              specName: data1.specName,
              trackDisposeInfo: data1.trackDisposeInfo,
              untowardEffectInfo: data1.untowardEffectInfo,
              untowardEffectType: data1.untowardEffectType,
              hospitalMemberName: data1.hospitalMemberName,
              hospitalMemberId: data1.hospitalMemberId,
            };
    ...
    orderDelete() {
      if (this.$route.query.id) {
        HarmfulResponseTest.destoryTrade(this.$route.query.id).then((res) => {
          if (res.data.code === 0) {
            this.$message({
              type: 'success',
              message: '删除成功',
