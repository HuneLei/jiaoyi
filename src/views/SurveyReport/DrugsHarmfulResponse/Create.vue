<template>
  <div v-loading="loading" class="drugsHarmfulResponse">
    ...
    <TradeFooter class="trade" :authbtn="authbtn" :isSaveShow="isSaveShow" :isBackShow="true" @order-save="orderSave('form')" @order-back="orderBack">
    </TradeFooter>
...
export default {
  ...
  mounted() {
    window.onresize = () => {
      if (this.$route.path === '/SurveyReport/DrugsHarmfulResponse/Create') {
  ...
  methods: {
    ...
    dataInit() {
      ...
      // };
      this.tableData = [];
      // this.form.hospitalMemberName = this.getPrincipal.memberName;
      // this.form.hospitalMemberId = this.getPrincipal.memberId;
      DrugsHarmfulResponse.hospitallist().then((res) => {
        if (res.data.code === 0) {
