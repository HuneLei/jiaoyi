            ...
            <el-table :data="NotDeliveryListAll[index]" stripe border fit class="from-table" :height="(NotDeliveryListAll[index].length * 32) + 55">
              ...
              <el-table-column align="right" min-width="80" label="发货数量" show-overflow-tooltip fixed="right">
                <template scope="scope">
                  <el-input :disabled="(NotDeliveryListAll.length-1) == index? disabled[index] : disabledNum[index]" :class="scope.row.OutStyle" class="input-right close-clear" style="width: 98%" slot="reference" size="small" placeholder="请输入内容" @focus="creatFocus(scope.row.drugsNum)" @keyup.native="keydown(scope.row.drugsNum, scope.row.hospitalUnitPrice, scope.$index, scope.row.purchaseNum, scope.row.BatchNum, index)" @blur="keydown(scope.row.drugsNum, scope.row.hospitalUnitPrice, scope.$index, scope.row.purchaseNum, scope.row.BatchNum, index)" v-model="scope.row.drugsNum">
                  </el-input>
                </template>
              </el-table-column>
              <el-table-column min-width="130" label="批号" fixed="right">
                <template scope="scope">
                  <el-select filterable :disabled="(NotDeliveryListAll.length-1) == index? disabled[index] : disabledNum[index]" v-model="batchNo[index][scope.$index]" placeholder="请选择" size="small" style="width: 98%" @change="BatchChange(batchNo[index][scope.$index], scope.$index, scope.row.drugsCode, index)" @visible-change="(value) => visibleChange(value, scope.row.drugsCode, index, scope.$index)" clearable>
              ...
              <el-table-column prop="contractItemNo" min-width="135" label="合同明细编号" show-overflow-tooltip>
              </el-table-column>
...
import YUploadEdit from '../../../components/YUploadEdit';                // 附件上传组件
  ...
  methods: {
    ...
    dialogOpen() {
      ...
      for (let i = 0; i < this.NotDeliveryListAll[0].length; i += 1) {
        ...
        this.NotDeliveryListAll[0][i].batchNo = this.batchNo[0][i];
        this.NotDeliveryListAll[0][i].makeDate = this.makeDate[0][i];
        this.NotDeliveryListAll[0][i].validDate = this.validDate[0][i];
        this.NotDeliveryListAll[0][i].drugsReportFile = this.drugsReportFile[0][i];
      ...
      this.Order_PlanfromEdit.sendPerson = this.NotDeliveryListAll[0][0].deliverPersons || '';
      this.Order_PlanfromEdit.sendPhone = this.NotDeliveryListAll[0][0].deliverPhones || '';
    ...
    dataSubmit() {
      ...
      if (bPass) {
        ...
        DeliveryOrdersList.delivery(from).then((res) => {
          if (res.data.code === 0) {
            this.$message({
              type: 'success',
              message: '发货成功',
