<template>
  <div v-loading="loading">
    <el-table :data="tableData" stripe border fit :height="View_TableHeight" @sort-change="sortChange">
      ...
      <el-table-column prop="amount" min-width="90" height="20px" label="订单金额" align="right" sortable="custom" show-overflow-tooltip>
        <template scope="scope">
      ...
      <el-table-column prop="noOut" width="110" height="20px" label="外部订单编号" sortable="custom" class-name="table_right" show-overflow-tooltip>
      </el-table-column>
      <el-table-column label="操作" min-width="100" max-width="200" fixed="right" align="center" v-if="authbtn.item">
        <template scope="scope">
          <router-link :to="{ path: '/Order/HospitalOrdersList/CheckHItem', query: { id: scope.row.id, status: scope.row.status, closeStatus: scope.row.closeStatus } }">
...
export default {
  ...
  methods: {
    ...
    dataList(page, pageSizes, formInline) {
      ...
      HospitalOrdersList.list(cpage, cpageSizes, form).then((res) => {
        if (res.data.code === 0) {
          this.tableNum = [];
          this.tableCount = 0;
