      ...
      <div v-loading="loading">
        ...
        <div>
          <el-table :data="NotDeliveryListAll" stripe border fit :height="300">
            ...
            <el-table-column prop="hospitalUnitPrice" min-width="70" label="单价" sortable show-overflow-tooltip>
            </el-table-column>
            ...
            <el-table-column min-width="80" label="发货数量" sortable show-overflow-tooltip fixed="right">
              <template scope="scope">
                <el-input :class="scope.row.OutStyle" class="close-clear input-right" style="width: 98%" slot="reference" size="small" placeholder="请输入内容" @focus="creatFocus(scope.row.drugsNum)" @keyup.native="keydown(scope.row.drugsNum, scope.row.hospitalUnitPrice, scope.$index, scope.row.purchaseNum, scope.row.BatchNum)" @blur="keydown(scope.row.drugsNum, scope.row.hospitalUnitPrice, scope.$index, scope.row.purchaseNum, scope.row.BatchNum)" v-model="scope.row.drugsNum">
                </el-input>
              </template>
            </el-table-column>
            <el-table-column min-width="100" label="批号" fixed="right">
              <template scope="scope">
                <el-select v-model="batchNo[scope.$index]" placeholder="请选择" size="small" style="width: 98%" @change="BatchChange(batchNo[scope.$index], scope.$index, scope.row.drugsCode)" @visible-change="(value) => visibleChange(value, scope.row.drugsCode, scope.$index)" clearable>
            ...
            <el-table-column prop="contractItemNo" min-width="160" label="合同明细编号" sortable show-overflow-tooltip>
            </el-table-column>
            <el-table-column label="操作" min-width="130" fixed="right" align="center">
              <template scope="scope">
                <el-button type="text" size="small" @click="batchNoPart(scope.$index)" v-if="!scope.row.operate" :disabled="scope.row.RowDisabled">
                  拆分批号
                </el-button>
                <el-button type="text" @click="batchNoDeletes(scope.row.num)" v-if="scope.row.num">
                  删除
                </el-button>
                <el-button type="text" size="small" @click="batchNoDelete(scope.$index)" v-if="scope.row.operate">
                  删除
                </el-button>
              </template>
            </el-table-column>
...
export default {
  ...
  methods: {
      ...
      if (this.NotDeliveryListAll) {
        for (let i = 0; i < this.NotDeliveryListAll.length; i += 1) {
          try {
            ...
            this.NotDeliveryListAll[i].batchNo = '';
            this.NotDeliveryListAll[i].makeDate = '';
            this.NotDeliveryListAll[i].validDate = '';
            this.NotDeliveryListAll[i].drugsReportFile = '';
    ...
    dataSubmit() {
      ...
      this.loading = true;
      DeliverBillList.notdelivery(from).then((res) => {
        if (res.data.code === 0) {
          this.$message({
            type: 'success',
            message: '发货成功',
