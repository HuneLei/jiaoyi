  ...
  export default {
    ...
    data() {
      return {
        ...
        editMessage: {
          dialogTitle: '修改卫监码',
          selects: [
            { label: '卫监码', key: 'superviseCode', type: 'el-input', placeholder: '请输入卫监码' },
          ],
          options: {},
          checkbox: {},
          radios: {},
          rules: {
            superviseCode: [
              { required: true, message: '请输入卫监码', trigger: 'blur' },
    ...
    methods: {
      ...
      dataCreate(row) {
        ...
        DrugsSuperviseCode.item(row.id).then((res) => {
          if (res.data.code === 0) {
            const data = res.data.result;
            this.form = {
              id: data.id,
              superviseCode: data.superviseCode,
            };
      ...
      dialogConfirm(formName) {
        DrugsSuperviseCode.update(this.form.id, this.form.superviseCode)
        .then((res) => {
          if (res.data.code === 0) {
