 ...
 * 公共转换js
 */
import pinyinJSON from './pinyinJSON';
...
//  获得一个文字的拼音
const getPinYin = (str) => {
  const keys = Object.keys(pinyinJSON.pinyinJSON);
  for (let i = 0; i < keys.length; i += 1) {
    const key = keys[i];
    const item = pinyinJSON.pinyinJSON[key];
...
//  获得一串文本的首拼
const getFirstPin = (str) => {
  let ret = '_';
  if (str || str === 0) {
    //  验证是否纯数字，包含+-号与小数
    if (/^[-+]?\d+\.?\d{0}$/.test(str)) {
      return `${ret}${10000000 + parseFloat(str)}`;
    }
    if (str.length) {
 ...
 */
const tableSort = (rule, tableData) => {
  const pinyin = `${new Date().getTime()}pinyin`; //  首拼标识

  const data = tableData;

  const arr = []; // 先获取首拼与首拼排序数组

  const arrNum = []; //  如果数组中num，把num排序正常放出来

  const arrSomeKey = []; //  相同的排序号的列
  ...
  for (let i = 0; i < tableData.length; i += 1) {
    if (rule.prop) {
      ...
      let shoupin = getFirstPin(data[i][rule.prop]);

      if (rule.order && rule.order === 'descending') {
        shoupin = `${shoupin}${1000000 + (tableData.length - i)}`;
      } else {
        shoupin = `${shoupin}${1000000 + i}`;
      }
      // console.log('shoupin', shoupin);

      arr.push(shoupin);
      data[i][pinyin] = shoupin;
    }
  }
  ...
  const numIndex = getArrayMin(arrNum);

  const dataRet = [];
  for (let i = 0; i < tableData.length; i += 1) {
    const item = tableData[i];
    const index = arr.indexOf(data[i][pinyin]);
    dataRet[index] = {
      ...item,
      num: numIndex + index,
    };

    delete dataRet[index][pinyin];
  }
